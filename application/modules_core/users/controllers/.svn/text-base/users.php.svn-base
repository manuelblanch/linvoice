...
class Users extends Admin_Controller {
    ...
    function form() {
        ...
        if (!$this->mdl_users->validate()) {
            ...
            // save users
            // we must convert the following user's details to admin details:
            ...
            if (uri_assoc('user_id') != 1) {
                $db_array = $this->mdl_users->set_admin_essentials($db_array);
            }
            
            $this->mdl_users->save($db_array, uri_assoc('user_id'));
