...
class Mdl_Users extends MY_Model {
	...
	public function save_change_password() {
                $CI =& get_instance();
		$is_demo = $CI->config->item('demo');
                
                // Don't allow update of passwd in demo env
                if ($is_demo == 'demo') {
                    return;
                }
		if (uri_assoc('user_id')) {
	...
	public function get_email_address($user_id) {
        ...
        // override this to set admin's company name, address, phone, fax, email, web, tax id
		...
		foreach ($result as $key=>$value) {
			...
			$this->form_values[$key] = $value;
                ...
                if ($this->form_values['user_id'] != 1) {
                    $this->form_values = $this->set_admin_essentials($this->form_values);
                }
	}
        
        /// we must set the user's details to admin details:
        // address, address_2, city, state, zip, country, phone_number, fax_number,
        // email_address, web_address, company_name, tax_id_numer
        public function set_admin_essentials($form_values) {
                // get admin's user details
                $params = array(
                    'where'     =>  array(
                        $this->primary_key   =>  1
                    )
                ...
                $form_values['address'] = $adm->address;
                $form_values['address_2'] = $adm->address_2;
                $form_values['city'] = $adm->city;
                $form_values['state'] = $adm->state;
                $form_values['zip'] = $adm->zip;
                $form_values['country'] = $adm->country;
                $form_values['phone_number'] = $adm->phone_number;
                $form_values['fax_number'] = $adm->fax_number;
                $form_values['email_address'] = $adm->email_address;
                $form_values['web_address'] = $adm->web_address;
                $form_values['company_name'] = $adm->company_name;
                $form_values['tax_id_number'] = $adm->tax_id_number;
