...
class Invoice_API extends Admin_Controller {
	...
	function create_invoice($package) {
		...
		$this->load->model('invoices/mdl_fields');
		$invoice_custom_fields = $this->mdl_fields->get_object_fields(1);

		$this->load->model('invoices/mdl_invoices');

		$invoice_id = $this->mdl_invoices->save($client_id, $invoice_date_entered, $invoice_is_quote, TRUE, $invoice_custom_fields );
